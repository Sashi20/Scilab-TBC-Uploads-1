...
//Example : 11.7
//Finding the Performance Parameters of a Single-Phase Cycloconverter with a Cosine Reference Signal.

clc;
clear;
Vs=120;                         //Supply voltage in volts.
fs=60;                          //Supply frequency in Hz.
fo=20;                          //Frequency in Hz.
R=5;                            //Resistance in Ohm.
L=40;                           //Inductance in milli Henry.
alp=(2*%pi)/3;                    //Delay angle in radians.
wo=2*%pi*fo;                    //Calculating frequency in rad/sec.
XL=wo*L*10^-3;                  //Calculating the reactance.
Vo=((2*Vs)/%pi);                //Calculating output voltage.
...
printf("\n\t(b).The input impedence Z is %0.2f Ohm",Z);
printf("\n\t    The angle ang is %0.1f degree",theta*180/%pi);
...
//Some Answers may vary due to round-off error.
